  ...
  module V1
    class TestJobFilesController < ApiController
      ...
      def bind_next_pending
        preferred_file_sql = current_project.test_jobs.
          where(status: [TestStatus::RUNNING,TestStatus::PENDING]).
          joins(:test_job_files).
          where(test_job_files: { status: TestStatus::PENDING }).
