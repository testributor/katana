class ProjectWizard < ActiveRecord::Base
  ...
  def to_project
    ...
    Project.transaction do
      project = user.projects.find_or_create_by!(name: repo_name) do |_project|
        _project.user                = user
        ...
        _project.repository_slug     = repository_slug
      end
      project.project_files.create!(path: "testributor.yml", contents: testributor_yml)
  ...
  def create_branches
    project = user.projects.find_by!(name: repo_name)
